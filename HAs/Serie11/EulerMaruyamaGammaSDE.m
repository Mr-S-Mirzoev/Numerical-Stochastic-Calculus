...
% coefficients of the Gamma subordinator
alpha = 2;
beta = 1; 
...
% generate K sample paths of a Gamma subordinator at the finest grid points
% GS =[zeros(1,K);cumsum(gamrnd((T/Nmax)*alpha,1/beta,Nmax,K))];

%%% Note that gamrnd takes the inverse rate parameter as input %%% 
...
GS =[zeros(1,K);cumsum(random('stable',1,0,(T/Nmax)*1,0,Nmax,K))];
...
for l=1:L+2			
    ...
    GSpart = GS(part, :); % to extract K sample paths of the Gamma subordinator at the grid points with time step size 1/(N0*2^le)
    YT(l,:) = EulerMaruyamaLevySDE(T, xi, mu_fun, sigma_fun, eta_fun, Wpart, GSpart); % K realizations of the EulerMaruyama approximation of X at t=T with time step size 1/(N0*2^le)
end
